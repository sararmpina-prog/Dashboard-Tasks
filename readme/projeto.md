...
Funções de renderização de cada elemento individual `renderUserCard()` e `createSingleTask()` seguida por a função de renderização de todos os elementos `renderUtilizadores()` e `renderTasks()`.

Tentei criar funções separadas para a criação de cada elemento botão por exemplo, `createBtnActivateToggle()`, acompanhadas pela função que chamo, neste caso, `switchUserState()` após associado evento (`addEventListener`). 
...
Neste momento, para a execução, o ideal será testar as funcionalidades dentro de cada página (ainda não existe uma interligação directa entre cada utilizador e as suas tarefas).
